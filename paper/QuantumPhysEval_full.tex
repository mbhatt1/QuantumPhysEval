...
\title{QuantumPhysEval: Measuring Exponential Reasoning Collapse in LLMs}
...
\item \textbf{Complexity scaling}: matched 4-qubit and 8-qubit reruns show that error grows with Hilbert-space size and that the growth is concentrated in dynamics-heavy tasks rather than in the fixed control slice.
\end{itemize}

\subsection{The structure of this paper}
The rest of the paper is organized as follows. First, we describe QuantumPhysEval's benchmark construction and scoring methodology. Next, we describe the case study protocol used to evaluate pooled OpenAI GPT and Anthropic Claude models. We then present top-level results, category-level findings, matched complexity-scaling stress tests, and repair ablations. Finally, we discuss limitations and conclude.
...
\begin{enumerate}
\item All studied models exhibit exponential reasoning collapse at non-trivial rates.
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.98\linewidth]{../artifacts/figures/combined_quantum_scaling.png}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.98\linewidth]{../artifacts/figures/combined_all_qubits_scaling.png}
\caption{Pooled scaling behavior across 2, 4, and 8 qubits. Top: the pooled mean, GPT-family mean, and Claude-family mean all worsen as the state space grows. Bottom: the sharpest increases remain concentrated in circuit evolution and measurement prediction, while operator composition stays comparatively flat as a control slice.}
\label{fig:qubit-stress}
\end{figure}

\begin{figure}[tbp]
\centering
\includegraphics[width=0.96\linewidth]{../artifacts/figures/combined_4qubit_breakdown.png}
\caption{Benchmark-family breakdown in the pooled 4-qubit stress test. Top: category difficulty after moving to 4 qubits. Bottom: model-by-category heatmap showing that the stress increase is again concentrated in circuit evolution and measurement prediction.}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.98\linewidth]{../artifacts/figures/anthropic_complexity_scaling.png}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.98\linewidth]{../artifacts/figures/anthropic_2q_vs_8q.png}
\caption{Anthropic 2-qubit versus 8-qubit stress comparison in the same visual format as the smaller stress-test figures. Left: both Claude models worsen substantially by 8 qubits. Right: the largest 2q$\rightarrow$8q increases remain circuit evolution and measurement prediction, while operator composition stays flat as a control.}
\label{fig:qubit-stress-8q}
\end{figure}

\begin{figure}[tbp]
\centering
\includegraphics[width=0.96\linewidth]{../artifacts/figures/combined_8qubit_breakdown.png}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.96\linewidth]{../artifacts/figures/gpt_8qubit_breakdown.png}
\caption{Completed GPT 8-qubit run. Left: category difficulty on the 5{,}374 usable-response file. Right: model-by-category heatmap. The same pattern visible in the Anthropic ladder persists here: circuit evolution and measurement prediction saturate, while operator composition remains substantially lower.}
\label{fig:gpt-8q-checkpoint}
\end{figure}

\begin{figure}[tbp]
\centering
\includegraphics[width=0.98\linewidth]{../artifacts/figures/gpt_complexity_scaling.png}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.98\linewidth]{../artifacts/figures/feedback_ablation_summary.png}
...
First, exponential reasoning collapse is measurable in a fully automated setting. Many outputs are syntactically clean and machine-readable, yet remain physically wrong under exact evaluation. This makes the benchmark useful as a reliability diagnostic rather than merely as a formatting test.
...
Fifth, the new complexity-scaling results show that these failures do not remain confined to the easiest possible small systems. The matched 4-qubit stress test already shows material degradation across both GPT and Claude families, and the completed Anthropic 8-qubit ladder strengthens that story further: error continues to rise with state-space size, exact-correct rate falls, full-error rate rises, and the degradation remains concentrated in the same dynamics-heavy categories. \textbf{There is a structural failure mode in LLMs tied to exponential state spaces.}

\section{Limitations}
QuantumPhysEval is intentionally narrow. It uses controlled small-system settings to enable exact scoring, but that also limits ecological breadth. Even with the added 4-qubit stress test and the completed Anthropic and GPT 8-qubit ladders, the benchmark remains far from realistic chemistry, control, or error-correction workloads. The model-size variable is a manually assigned proxy rather than a verified parameter count, and the pooled fit mixes families and release generations. The benchmark also uses a single prompt style per task family and a single temperature setting. The larger-qubit stress tests do not upscale operator composition, since returning exact 16$\times$16 or 64$\times$64 matrices would mostly measure formatting burden rather than useful reasoning. The repair ablation is smaller than the pooled study and uses a binary verifier signal that would not always be available in practical deployments. The completed GPT 8-qubit file still contains two residual API failures, but at 5{,}374 usable responses the qualitative pattern is stable and matches the Anthropic ladder. The present evidence therefore supports a strong benchmark-level capability trend, but it should not be overinterpreted as a universal scaling law.

\section{Conclusion}
In this paper, we introduced QuantumPhysEval, a benchmark for measuring exponential reasoning collapse as physics-grounded error on elementary quantum tasks. In a pooled 2-qubit case study over OpenAI GPT and Anthropic Claude models, benchmark error decreases strongly with model capability, while prompted reasoning depth shows no robust independent effect. A matched 4-qubit stress test shows that performance worsens materially once the state space expands, especially on circuit evolution and measurement prediction. A completed Anthropic 8-qubit ladder shows that this is a genuine complexity trend rather than a one-step artifact: error continues to rise, exact-correct rate falls, and full-error failures become more common as the Hilbert space grows. Separate repair ablations show that repeated attempts can recover some failures, especially when verifier feedback is available, but those gains saturate quickly and do not erase the main weakness. \textbf{There is a structural failure mode in LLMs tied to exponential state spaces.} These results suggest that language models can produce plausible quantum answers, but exact physical reliability remains limited and should be measured explicitly rather than assumed.
